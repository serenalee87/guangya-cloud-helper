...
- `TMDB API Key`
  媒体智能整理使用。登录 `https://www.themoviedb.org/` 后进入 `Settings -> API`，申请 `Developer / v3 API Key`，复制生成的 Key 填入即可。不填时仍可使用本地规则粗分。

- `整理根目录 parentId`
  媒体智能整理使用。留空默认用当前目录；填写后会在指定媒体库根目录下创建分类目录。
